...
_ADDRESS_OR_ID_INPUT_SCHEMA = {
    ...
    "properties": {
        "address": {
            "type": "string",
            "description": "Full street address string for getByAddress mode.",
        },
        "id": {"type": "string", "description": "Entity identifier for getById mode."},
        "queryType": {
            "type": "string",
            "description": "ID namespace for getById mode.",
            "enum": [
                "PRECISELY_ID",
                "PARCEL_ID",
                "BUILDING_ID",
...
Output: GraphQL response with address data matching the requested fields. Structure depends on the query provided.""",
            inputSchema={
                "type": "object",
                "properties": {"data": _GRAPHQL_DATA_SCHEMA},
                "required": ["data"],
            },
        ),
        Tool(
...
{'data': {
  ...
  'variables': {'address': '123 Main St, Boston, MA 02101', 'country': 'US'}
        ...
        Tool(
            name="get_datalink_dun_bradstreet",
            description="""Retrieve Data Link Dun & Bradstreet records using a merged query mode.

Mode 1 (by address): provide 'address' only.
Mode 2 (by ID): provide 'id' and 'queryType' only.

This tool sends curated GraphQL to the Data Graph endpoint and returns metadata + records from
datalinkDunAndBradstreet.

Fields returned:
  placeId, vintage, linkId, linkIdVintage, linkIdType, preciselyId,
  preciselyIdVintage, parentPreciselyId

Notes:
- Data access may be permission-blocked depending on your Precisely credential tier.
- Do not send address and id/queryType together in one call.""",
